...
export default () => {
  ...
  const sieve = (filter) => {
    const sieve_inner = (offer) => {
      const filter_search = () => {
        ...
        if (offer.code.toLowerCase().includes(filter.search.toLowerCase())) {
          return true;
        }
        return false;
      };
  ...
  const handleDeleteOffer = (offer) => {
    const url =
      process.env.REACT_APP_API_URL + "/management/offer/" + offer.code;
  ...
  return (
    <section className="min-h-fscr bg-white bg-opacity-80 py-8">
